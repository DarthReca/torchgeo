...
class HydroChronosDataModule(NonGeoDataModule):
    mean = {
        'landsat': torch.tensor(
            [31.510, 29.306, 25.032, 62.795, 37.586, 21.682, 4.845]
        ),
        'sentinel': torch.tensor(
            [27.617, 24.782, 19.763, 60.604, 38.653, 21.459, 4.845]
        ),
    }
    std = {
        'landsat': torch.tensor(
            [18.089, 27.216, 27.928, 34.766, 26.263, 21.155, 0.848]
        ),
        'sentinel': torch.tensor(
            [20.668, 20.661, 23.357, 33.452, 25.807, 19.753, 0.848]
        ),
    }
    ...
    def __init__(self, cls, num_workers: int = 0, batch_size: int = 1, **kwargs):
        ...
        self.train_aug = K.AugmentationSequential(
            D4(),
            K.RandomAffine(degrees=0, translate=(0.1, 0.1), scale=(0.5, 1.0), p=1.0),
